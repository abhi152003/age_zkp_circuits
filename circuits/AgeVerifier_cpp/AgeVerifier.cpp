...
void AgeVerifier_2_create(uint soffset,uint coffset,Circom_CalcWit* ctx,std::string componentName,uint componentFather);
void AgeVerifier_2_run(uint ctx_index,Circom_CalcWit* ctx);
...
// load src
Fr_sub(&expaux[0],&signalValues[mySignalStart + 2],&signalValues[mySignalStart + 1]); // line circom 17
// end load src
Fr_copy(aux_dest,&expaux[0]);
}
{
...
// load src
cmp_index_ref_load = 0;
Fr_sub(&expaux[0],&circuitConstants[2],&ctx->signalValues[ctx->componentMemory[mySubcomponents[0]].signalStart + 0]); // line circom 24
// end load src
Fr_copy(aux_dest,&expaux[0]);
}
for (uint i = 0; i < 1; i++){
...
AgeVerifier_2_create(1,0,ctx,"main",0);
AgeVerifier_2_run(0,ctx);
}
